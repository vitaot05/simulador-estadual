...
function sanitizeTeam(t) {
  return {
    id: t.id,
    name: t.name,
    reputation: clamp(Number(t.reputation || 1), 1, 7),
    rating: Number(t.rating || generateRatingFromReputation(t.reputation || 1)),
    form: Number(t.form || 0),
  };
}
function effectiveRating(t) {
  return clamp(
    Number(t.rating || generateRatingFromReputation(t.reputation || 3)) +
      Number(t.form || 0),
    35,
    99,
  );
...
function allTeams() {
  return Object.values(game.divisions || {})
    .flat()
    .map(sanitizeTeam);
}
...
function team(league, id) {
  return game.divisions[league]?.find((t) => t.id === id) || findTeam(id);
}
function maxRounds() {
  return Math.max(
    0,
    ...Object.values(game.schedules || {}).map((s) => s.length),
  );
}
function cupSize(total) {
...
function cupRoundName(size, remaining) {
  if (remaining === 2) return "Final";
  if (remaining === 4) return "Semifinal";
  if (remaining === 8) return "Quartas";
  if (remaining === 16) return "Oitavas";
  return `${remaining / 2}ª fase`;
}
function cupRoundCount() {
  return game?.cup?.size ? Math.log2(game.cup.size) : 0;
}
function nextActionLabel() {
  return game.round >= maxRounds() && (!game.cup?.size || game.cup?.winner)
    ? "Encerrar"
    : "Simular";
}
function shouldPlayCupNow() {
  return game.cup?.size && !game.cup?.winner && game.phase === "cup";
}
function shouldPlaySupercupNow() {
  return game.phase === "supercup" && game.pendingSupercup && !game.supercup;
}
function cupRoundsTotal() {
  return game?.cup?.size ? Math.log2(game.cup.size) : 0;
}
function cupRoundsPlayed() {
  return game?.cup?.history?.length || 0;
}
function cupHasOnlyFinalLeft() {
  return (
    game?.cup?.size &&
    !game.cup?.winner &&
    cupRoundsPlayed() >= cupRoundsTotal() - 1
  );
}
...
function createGame(config, slot, shouldShuffle = false) {
  ...
  Object.keys(divisions).forEach(
    (l) => (divisions[l] = divisions[l].map(sanitizeTeam)),
  );
  const built = buildSeason(divisions);
  const g = {
    id: "slot_" + slot,
    slot: Number(slot),
    name: `Slot ${slot} • ${cfg.zone}`,
    config: cfg,
    zone: cfg.zone,
    season: 2026,
    round: 0,
...
function buildSeason(divisions) {
  ...
  for (const [league, teams] of Object.entries(divisions)) {
    tables[league] = newTable(teams.map(sanitizeTeam));
...
function buildCup() {
  ...
  const seeded = teams
    ...
    .sort(
      (a, b) =>
        effectiveRating(b) - effectiveRating(a) || a.name.localeCompare(b.name),
    )
    .slice(0, size);
  const mixed = [];
  for (let i = 0; i < size / 2; i++) {
    mixed.push(seeded[i], seeded[size - 1 - i]);
  }
  return {
    name: game.config.cup,
    size,
    active: true,
    participants: mixed.map((t) => t.id),
    roundIndex: 0,
    history: [],
    winner: null,
    runnerUp: null,
...
function currentRoundMatches() {
  const out = [];
  for (const league of Object.keys(game.schedules || {})) {
    (game.schedules[league][game.round] || []).forEach(([homeId, awayId]) =>
      out.push({
        league,
        home: team(league, homeId),
        away: team(league, awayId),
      }),
    );
  }
  return out;
...
function simKnockout(home, away) {
  let [hg, ag] = simMatch(home, away, true),
    penalties = null;
  if (hg === ag) {
    const hp = clamp(Math.round(rand(3, 6)), 0, 7),
      ap = clamp(Math.round(rand(2, 6)), 0, 7);
...
function applyResult(league, home, away, hg, ag) {
  const h = game.tables[league][home.id],
    a = game.tables[league][away.id];
  if (!h || !a) return;
  h.played++;
  a.played++;
  h.goalsFor += hg;
...
function movementInfo(league, i, count) {
  const leagues = game.config.leagues,
    idx = leagues.indexOf(league);
  if (idx > 0 && i < PROMOTION_SLOTS)
    return {
      cls: "pos-promo",
      label: "Acesso",
      title: `Sobe para ${leagues[idx - 1]}`,
    };
  if (idx < leagues.length - 1 && i >= count - RELEGATION_SLOTS)
    return {
      cls: "pos-releg",
      label: "Queda",
      title: `Cai para ${leagues[idx + 1]}`,
    };
...
function stripTeam(t) {
  ...
  return {
    ...
    rating: clamp(
      Math.round(
        Number(t.rating || generateRatingFromReputation(rep)) + rand(-2, 2),
      ),
      35,
      99,
    ),
...
function addTitle(teamName, competition) {
  ...
  game.museum[key].byCompetition[competition] =
    (game.museum[key].byCompetition[competition] || 0) + 1;
  game.museum[key].byLeague[competition] =
    (game.museum[key].byLeague[competition] || 0) + 1;
}
...
function isLeagueCompetition(name) {
  return (game.config.leagues || []).includes(name);
}
function isCupCompetition(name) {
  return name === game.config.cup;
}
function isSupercupCompetition(name) {
  return name === game.config.supercup;
}

function currentViewSeason() {
  const seasons = seasonOptions();
  if (!activeViewSeason || !seasons.includes(Number(activeViewSeason)))
    activeViewSeason = game.season;
...
function renderSeasonSelect() {
  ...
  sel.innerHTML = seasons
    .map(
      (y) =>
        `<option value="${y}" ${Number(y) === Number(activeViewSeason) ? "selected" : ""}>${y}</option>`,
    )
    .join("");
  sel.onchange = () => {
    activeViewSeason = Number(sel.value);
    activeResultsRound = null;
    render();
  };
}
...
function renderCompetitionSelect() {
  ...
  activeCompetition = opts.includes(activeCompetition)
    ? activeCompetition
    : opts[0];
  sel.innerHTML = opts
    .map(
      (l) =>
        `<option value="${escapeHTML(l)}" ${l === activeCompetition ? "selected" : ""}>${escapeHTML(l)}</option>`,
    )
    .join("");
  sel.onchange = () => {
    activeCompetition = sel.value;
    activeResultsRound = null;
    render();
  };
}
...
function standingsTable(league) {
  ...
  return `<div class="tbl-wrap"><table class="standings-table"><thead><tr><th class="pos-col">#</th><th class="club-col">Clube</th><th>Pts</th><th>J</th><th>V</th><th>E</th><th>D</th><th>GP</th><th>GC</th><th>SG</th></tr></thead><tbody>${rows
    .map((t, i) => {
      const mv = movementInfo(league, i, rows.length);
      return `<tr class="${mv.cls}"><td class="pos-col">${i + 1}</td><td class="club-col">${escapeHTML(t.name)}</td><td class="pts">${t.points}</td><td>${t.played}</td><td>${t.won}</td><td>${t.drawn}</td><td>${t.lost}</td><td>${t.goalsFor}</td><td>${t.goalsAgainst}</td><td class="${t.goalDifference > 0 ? "good" : t.goalDifference < 0 ? "bad" : ""}">${t.goalDifference}</td></tr>`;
    })
...
function renderCompetition() {
  ...
  if (isLeagueCompetition(comp)) {
    ...
    if (Number(season) !== Number(game.season)) {
      box.innerHTML = finals?.[comp]
        ? competitionTitle(comp) + renderMiniTable("", finals[comp] || [])
        : '<p class="empty-text">Sem tabela final para esta temporada.</p>';
    } else {
  ...
  if (isCupCompetition(comp)) {
    ...
    if (cupList.length) {
      const grouped = {};
      cupList.forEach((r) => {
        (grouped[r.name || comp] ||= []).push(r);
      });
      const keys = Object.keys(grouped);
      box.innerHTML = keys
        .map(
          (name) =>
            `<section class="round-results"><h3>${escapeHTML(name)}</h3><div class="results-list">${grouped[name].map(matchLine).join("")}</div></section>`,
        )
        .join("");
    } else {
      const cup =
        Number(season) === Number(game.season) ? game.cup || buildCup() : null;
      const status =
        cup && cup.size
          ? cup.winner
            ? `Campeão: ${cup.winner}`
            : `Próxima fase: ${cupRoundName(cup.size, cup.participants.length)}`
  ...
  box.innerHTML = sc
    ? `<div class="results-list">${matchLine(sc)}</div><p class="cup-winner">Campeão: ${escapeHTML(sc.winner)}</p>`
    : `<p class="empty-text">Sem supercopa disputada.</p>`;
}
function seasonOptions() {
  const seasons = [game.season, ...(game.history || []).map((h) => h.season)]
...
function resultOptionsForCompetition(season, comp) {
  if (isLeagueCompetition(comp)) {
    ...
    const rounds = [
      ...new Set(
        resultsForSeason(season)
          .filter((r) => r.league === comp)
          .map((r) => Number(r.round)),
      ),
    ].sort((a, b) => b - a);
    rounds.forEach((r) =>
      opts.push({ value: String(r), label: `Rodada ${r}` }),
    );
    return opts;
  }
  if (isCupCompetition(comp)) {
    const names = [
      ...new Set(cupResultsForSeason(season).map((r) => r.name || comp)),
    ].reverse();
    return names.map((n) => ({ value: n, label: n }));
  }
  if (isSupercupCompetition(comp))
    return supercupForSeason(season)
      ? [{ value: "supercup", label: comp }]
      : [];
  return [];
}
  ...
  if (isLeagueCompetition(comp)) {
    ...
    html = rounds.length
      ? rounds
          .map(
            (r) =>
              `<section class="round-results"><h3>Rodada ${r}</h3><div class="results-list">${byRound[r].map(matchLine).join("")}</div></section>`,
          )
          .join("")
      : '<p class="empty-text">Sem resultados.</p>';
  } else if (isCupCompetition(comp)) {
    const byPhase = {};
    cupResultsForSeason(season).forEach((r) => {
      const key = r.name || comp;
      (byPhase[key] ||= []).push(r);
    });
    ...
    html = phases.length
      ? phases
          .map(
            (name) =>
              `<section class="round-results"><h3>${escapeHTML(name)}</h3><div class="results-list">${byPhase[name].map(matchLine).join("")}</div></section>`,
          )
          .join("")
      : '<p class="empty-text">Sem resultados.</p>';
    ...
    const sc = supercupForSeason(season);
    html = sc
      ? `<section class="round-results"><h3>${escapeHTML(comp)}</h3><div class="results-list">${matchLine(sc)}</div><p class="cup-winner">Campeão: ${escapeHTML(sc.winner)}</p></section>`
      : '<p class="empty-text">Sem resultados.</p>';
  }
  box.innerHTML = html;
}
function calendarEvents() {
  ...
  if (hasSupercup) {
    ...
    const sub = game.supercup
      ? `${game.supercup.home} × ${game.supercup.away}`
      : `${base.mainChampion || "-"} × ${base.cupWinner || "-"}`;
    events.push({
      kind: "supercup",
      title: game.config.supercup || "Supercopa",
      sub,
      status: game.supercup ? "done" : "todo",
    });
  }
  const max = maxRounds();
  const total = cupRoundsTotal();
  ...
  for (let i = 1; i <= max; i++) {
    ...
    if (game.cup?.size && cupIdx < Math.max(0, total - 1)) {
      ...
      events.push({
        kind: "cup",
        title,
        sub: game.config.cup || "Copa",
        status: played ? "done" : "todo",
      });
      cupIdx++;
    }
  }
...
function formatCupScore(r) {
  const pen = r.penalties ? ` (${r.penalties[0]}-${r.penalties[1]} pen.)` : "";
  return `${r.homeGoals} × ${r.awayGoals}${pen}`;
}
function renderCups() {
  const box = document.getElementById("cups");
  const cup = game.cup || buildCup();
  const supercup = game.supercup;
  const cupStatus = !cup.size
    ? "Copa indisponível: mínimo de 8 clubes."
    : cup.winner
      ? `Campeão: ${cup.winner}`
      : `${cup.size} clubes · próxima fase: ${cupRoundName(cup.size, cup.participants.length)}`;
  const cupHistory =
    (cup.history || [])
      .slice()
      .reverse()
      .map(
        (round) =>
          `<section class="round-results"><h3>${escapeHTML(round.name)}</h3><div class="results-grid">${round.matches.map((r) => `<div class="match-line"><span>${escapeHTML(r.home)}</span><strong>${formatCupScore(r)}</strong><span>${escapeHTML(r.away)}</span></div>`).join("")}</div></section>`,
      )
      .join("") || '<p class="muted">Nenhum jogo de copa disputado ainda.</p>';
  const superHtml = supercup
    ? `<div class="cup-card"><h3>${escapeHTML(game.config.supercup)}</h3><p class="muted">${escapeHTML(supercup.home)} ${supercup.homeGoals} × ${supercup.awayGoals} ${escapeHTML(supercup.away)}</p><div class="cup-winner">${escapeHTML(supercup.winner)}</div></div>`
    : `<div class="cup-card"><h3>${escapeHTML(game.config.supercup)}</h3><p class="muted">Será disputada entre campeão da liga principal e campeão da copa.</p></div>`;
...
function renderMuseum() {
  ...
  const museum = game.museum || {};
  const competitions = [
    ...game.config.leagues,
    game.config.cup,
    game.config.supercup,
  ];
  const rows = Object.values(museum).sort(
    (a, b) => b.total - a.total || a.team.localeCompare(b.team),
  );
  if (!rows.length) {
    box.innerHTML = '<p class="muted">Nenhum título registrado ainda.</p>';
    return;
  }
...
function renderHistory() {
  ...
  const moves = (game.lastMovements || []).length
    ? `<div class="notice"><strong>Movimentações</strong>${game.lastMovements.map((m) => `<p class="muted">${m.type === "promotion" ? "↑" : "↓"} ${escapeHTML(m.team)}: ${escapeHTML(m.from)} → ${escapeHTML(m.to)}</p>`).join("")}</div>`
    : "";
  const seasons = game.history?.length
    ? game.history
        ...
        .map(
          (h) =>
            `<div class="save-card"><strong>${escapeHTML(game.zone)} · ${h.season}</strong>${Object.entries(
              h.winners,
            )
              .map(
                ([l, w]) =>
                  `<p class="muted">${escapeHTML(l)}: ${escapeHTML(w)}</p>`,
              )
              .join(
                "",
              )}${h.cupWinner ? `<p class="muted">${escapeHTML(game.config.cup)}: ${escapeHTML(h.cupWinner)}</p>` : ""}${h.supercupWinner ? `<p class="muted">${escapeHTML(game.config.supercup)}: ${escapeHTML(h.supercupWinner)}</p>` : ""}</div>`,
        )
        .join("")
    : '<p class="muted">Nenhuma temporada concluída.</p>';
...
function renderSettings() {
  ...
  if (auto && document.activeElement !== auto)
    auto.value = settings.autosaveFrequency || "1";
  if (speed && document.activeElement !== speed)
    speed.value = String(settings.simulationSpeed || 600);
}
function currentWeekLabel() {
  const done =
    (game.supercup ? 1 : 0) + Number(game.round || 0) + cupRoundsPlayed();
  let type = "Liga";
  if (shouldPlaySupercupNow()) type = game.config.supercup || "Supercopa";
  else if (shouldPlayCupNow()) type = game.config.cup || "Copa";
  else if (game.round >= maxRounds() && game.cup?.size && !game.cup.winner)
    type = game.config.cup || "Copa";
  else if (game.round >= maxRounds() && (!game.cup?.size || game.cup.winner))
    type = "Encerrar temporada";
  return `Semana ${done + 1}\n Próxima rodada: ${type}`;
}
function renderBottomNav() {
  ...
  const sim = document.getElementById("sim-round");
  if (sim) sim.textContent = nextActionLabel();
}
function render() {
  syncSimulationMode();
  renderSeasonSelect();
...
function simulateCupRound() {
  const cup = game.cup;
  if (!cup || !cup.size || cup.winner || cup.participants.length < 2) return;
  const remaining = cup.participants.length,
    roundName = cupRoundName(cup.size, remaining),
    next = [],
    matches = [];
  for (let i = 0; i < cup.participants.length; i += 2) {
    const home = findTeam(cup.participants[i]),
      away = findTeam(cup.participants[i + 1]);
    const r = simKnockout(home, away);
    next.push(r.winner.id);
    matches.push({
      season: game.season,
      name: roundName,
      home: home.name,
  ...
  if (next.length === 1) {
    cup.winner = findTeam(next[0]).name;
    const finalMatch = matches[0];
    cup.runnerUp = finalMatch
      ? finalMatch.winner === finalMatch.home
        ? finalMatch.away
        : finalMatch.home
      : null;
    addTitle(cup.winner, game.config.cup);
  }
}
function playPendingSupercup() {
  ...
  if (res) {
    game.supercup = res;
    game.supercup.season = game.season;
  }
  game.pendingSupercup = null;
  return res;
}
function simulationDelay() {
  return Number(getSettings().simulationSpeed || 600);
}
...
function currentSimulationStage() {
  return simulationContext?.stages?.[simulationContext.stageIndex] || null;
}
...
function buildCupSimulationContext() {
  const cup = game.cup;
  if (!cup || !cup.size || cup.winner || cup.participants.length < 2)
    return null;
  const remaining = cup.participants.length;
...
function buildSupercupSimulationContext() {
  const pending = game.pendingSupercup;
  if (!pending) return null;
  let mainChampion = pending.mainChampion,
    cupWinner = pending.cupWinner;
  if (!mainChampion || !cupWinner || mainChampion === "-" || cupWinner === "-")
    return null;
  let opponent = cupWinner;
  if (opponent === mainChampion && game.cup?.runnerUp)
    opponent = game.cup.runnerUp;
  if (opponent === mainChampion) return null;
  const home = allTeams().find((t) => t.name === mainChampion) || {
    id: mainChampion,
    name: mainChampion,
    rating: 78,
    reputation: 4,
  };
  const away = allTeams().find((t) => t.name === opponent) || {
    id: opponent,
    name: opponent,
    rating: 78,
    reputation: 4,
  ...
  return {
    type: "supercup",
    label: game.config.supercup || "Supercopa",
    matches: [
      {
        homeId: home.id,
        awayId: away.id,
...
function buildSimulationContext() {
  ...
  if (!ctx) {
    if (game.cup?.size && !game.cup.winner) {
      game.phase = "cup";
      ctx = buildCupSimulationContext();
...
function renderSimulationMatches(ctx, stage) {
  if (ctx.type === "seasonEnd")
    return '<p class="empty-text">A temporada será encerrada e os acessos/rebaixamentos serão aplicados.</p>';
  const line = (m) => {
    const score = stage === "warmup" ? "×" : `${m.homeGoals} × ${m.awayGoals}`;
    const pen =
      (stage === "penalties" || stage === "result") && m.penalties
        ? `<small>${m.penalties[0]}-${m.penalties[1]} pen.</small>`
        : "";
  ...
  if (ctx.type === "league") {
    ...
    return `<div class="simulation-groups">${Object.entries(grouped)
      .map(
        ([league, matches]) =>
          `<section class="simulation-group"><h3>${escapeHTML(league)}</h3><div class="simulation-list">${matches.map(line).join("")}</div></section>`,
      )
      .join("")}</div>`;
  }
...
function renderSimulation() {
  const box = document.getElementById("simulation-view");
  if (!box) return;
...
function commitLeagueSimulation(ctx) {
  ctx.matches.forEach((m) => {
    const h = team(m.league, m.homeId),
      a = team(m.league, m.awayId);
    applyResult(m.league, h, a, m.homeGoals, m.awayGoals);
    game.results.push({
      season: game.season,
      round: ctx.round,
      league: m.league,
...
function commitSimulation(ctx) {
  ...
  else if (ctx.type === "seasonEnd") {
    finishSeason();
    return;
  }
  game.updatedAt = new Date().toISOString();
  if (ctx.type === "cup" && game.cup?.winner && game.round >= maxRounds()) {
...
function skipToCalendarIndex(targetIndex) {
  ...
  const ok = confirm(
    `Simular automaticamente até a semana ${targetIndex + 1}?`,
  );
  if (!ok) return;
  clearSimulationTimer();
  simulationContext = null;
  ...
  while (completedCalendarEvents() <= targetIndex && guard < 250) {
    const before = JSON.stringify({
      ...
      round: game.round,
      phase: game.phase,
      cup: game.cup?.history?.length,
      winner: game.cup?.winner,
      supercup: !!game.supercup,
    });
...
function finishSeason() {
  ...
  leagues.forEach((league, idx) => {
    if (idx > 0)
      nextDivisions[league].push(
        ...(sorted[leagues[idx - 1]] || [])
          .slice(-RELEGATION_SLOTS)
          .map(stripTeam),
      );
  ...
  game.history.push({
    ...
    zone: game.zone,
    winners,
    cupWinner: game.cup?.winner || null,
    supercupWinner: null,
    supercup: null,
    movements,
    finalTables: Object.fromEntries(leagues.map((l) => [l, sorted[l] || []])),
    leagueResults: [...(game.results || [])],
    cupResults: [...(game.cupResults || [])],
  });
  game.season++;
  game.round = 0;
  game.pendingSupercup = {
    season: game.season,
    mainChampion: winners[leagues[0]],
    cupWinner: game.cup?.winner || null,
  };
  game.supercup = null;
  game.phase = game.pendingSupercup?.cupWinner ? "supercup" : "league";
  game.divisions = nextDivisions;
  const built = buildSeason(game.divisions);
  game.tables = built.tables;
  game.schedules = built.schedules;
  game.results = [];
  game.cupResults = [];
  game.lastMovements = movements;
  game.cup = buildCup();
  autosave(true);
...
async function init() {
  ...
  if (!game) {
    ...
    if (pending) {
      cfg = JSON.parse(pending);
      sessionStorage.removeItem("avanci_pending_config");
    }
    const errors = validateConfig(normalizeConfig(cfg));
    if (errors.length)
      alert("Configuração inicial inválida: " + errors.join(" "));
    const shouldShuffle = confirm(
      "Deseja embaralhar os times antes de dividir as ligas?",
    );
    game = createGame(cfg, slot, shouldShuffle);
    autosave(true);
  }
  game.config = normalizeConfig(game.config || (await loadDefaultConfig()));
  Object.keys(game.divisions || {}).forEach(
    (l) => (game.divisions[l] = game.divisions[l].map(sanitizeTeam)),
  );
  game.museum ||= {};
  if (!game.tables || !game.schedules) {
    const built = buildSeason(game.divisions);
    game.tables = built.tables;
    game.schedules = built.schedules;
  }
  ...
  game.history ||= [];
  game.pendingSupercup ||= null;
  if (game.pendingSupercup && !game.supercup) game.phase = "supercup";
  game.phase ||= "league";
  activeCompetition = competitionOptions().includes(activeCompetition)
    ? activeCompetition
    : competitionOptions()[0];
  activeViewSeason = game.season;
  document.getElementById("skip-round").onclick = skipCurrentWeek;
  document.getElementById("settings-btn").onclick = openSettings;
  document.getElementById("close-settings").onclick = closeSettings;
  document.getElementById("export-save").onclick = () =>
    downloadJSON(`${game.id}.json`, game);
  document.getElementById("import-save").onchange = async (e) => {
    if (!e.target.files[0]) return;
    try {
      const imported = await readJSONFile(e.target.files[0]);
      imported.slot = game.slot;
      imported.id = "slot_" + game.slot;
      game = imported;
      game.config = normalizeConfig(game.config);
      activeCompetition =
        game.config?.leagues?.[0] || Object.keys(game.divisions || {})[0];
      autosave(true);
      render();
      closeSettings();
    } catch (err) {
      alert("Save inválido: " + err.message);
    }
  };
  document.getElementById("autosave-frequency").onchange = (e) =>
    setSettings({ autosaveFrequency: e.target.value });
  document.getElementById("simulation-speed").onchange = (e) =>
    setSettings({ simulationSpeed: Number(e.target.value) });
  renderTabs();
  render();
